# 4. EfficientNetB0 (최신 기술)
# 특징: 현재 이미지 분류 대회에서 가장 많이 쓰이는 모델입니다. MobileNet보다조금 더 무겁지만, 정확도는 훨씬 높게 나올 가능성이 크다.
...
def make_model(input_shape, num_classes):
    ...
    # EfficientNetB0 불러오기
    # weights='imagenet': 이미 공부를 많이 한 모델
    ...
    # 모델 연결
    # EfficientNet은 내부에 전처리가 포함되어 있어서 별도 스케일링 불필요
    x = base_model(x, training=False)

    x = layers.GlobalAveragePooling2D()(x)
    x = layers.Dropout(0.3)(x)  # 성능이 좋은 만큼 과적합 방지 조금 더 세게
